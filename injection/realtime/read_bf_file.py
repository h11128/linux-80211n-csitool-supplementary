...
while True:
    ...
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)         # build socket object
    host1 = "127.0.0.1" # get local host name
    host = socket.gethostname()
    port = 8090       
    print(host1, port)         
    s.bind((host1, port))        # bind port
    
    s.listen(5)                 # waiting for connection on port
    print('waiting for connection on port', port)
    
    c, addr = s.accept()     # build client connection
    c.settimeout(15)
